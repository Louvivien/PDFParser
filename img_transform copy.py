...
def crop_central_line(image_path, output_path, padding=10):
    ...
    max_y_padded = min(max_y + padding, img.shape[0])
    min_x_padded = max(min_x - padding, 0)
    max_x_padded = min(max_x + padding, img.shape[1])

    # Extract the corresponding region with optional padding
    cropped_img = img[min_y_padded:max_y_padded, min_x_padded:max_x_padded]
